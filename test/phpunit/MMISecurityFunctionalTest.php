...
/**
 * \file    test/phpunit/MMISecurityFunctionalTest.php
 * \ingroup mmisecurity
...
/**
 * Class MMISecurityFunctionalTest
 *
 ...
 *
 * @package Testmmisecurity
 */
class MMISecurityFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMMISecurity')->click();
		} else {
			// Disable the module
			$this->byHref('modMMISecurity')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mmisecurity/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mmisecurity/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mmisecurity/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mmisecurity/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mmisecuritywidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMMISecurity_MMISecurityTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMMISecurity_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
